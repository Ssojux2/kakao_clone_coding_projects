...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(default_factory=list, description="분리된 구조화 요청 목록")
    ...
    return join_system_prompt(
        [
            *week02_prompt_parts(),
            ...
            "개인 일정 생성 요청에서 personal_create_schedule을 호출했다면 tool 결과 JSON의 created_schedule을 읽어 "
            "kind='personal_schedule', title/date/start_time/end_time/members/original_text를 채운다. "
            "설명 문장이나 JSON object를 반복 출력하지 않는다.",
        ]
    )
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        ...
        "사용자의 한국어 자연어 요청을 읽고 날짜, 시간, 제목, 멤버, 종류를 StructuredRequest 필드 기준으로 구조화한다. "
        "Week 2 대화의 최종 구조화는 response_format의 StructuredRequestBatch가 담당한다. "
        ...
        "사용자가 이미 Week 1 tool 결과 JSON을 전달했다면 tool을 다시 호출하지 말고 그 JSON을 읽어 structured_response를 만든다. "
        "할 일, 알림, 그룹 일정처럼 Week 1 생성 tool로 표현할 수 없는 요청은 원문 전체를 직접 읽어 structured_response를 만든다. "
        "Week 2에서는 SQLite 저장, RAG, 외부 멤버 일정 조율을 처리하지 않는다. "
        "Week 1의 tool 호출/자연어 답변 지시보다 Week 2의 structured output 계약을 우선한다. "
        "모르는 값은 억지로 만들지 말고 None 또는 빈 list로 둔다."
    ]
