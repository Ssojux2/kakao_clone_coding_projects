...
### 메인/추가 과제 분리 원칙 (Week 3 이후)

Week 3부터 각 주차 과제는 **메인 과제**와 **추가 과제**로 나뉩니다.

- **메인 과제만 구현해도 `./run.sh --weekN`이 끝까지 동작해야 합니다.** 추가 과제는 순수하게 얹는 기능이며, 미구현이어도 해당 주차 메인 기능이 완주합니다.
- **별도 등록 함수는 두지 않습니다.** 추가 과제 tool도 그대로 각 파일의 `weekNN_tools()`(Week 6은 `kana_tools()`) 목록에 들어갑니다. 추가 과제를 구현하지 않는 수강생은 **해당 tool을 `weekNN_tools()` 목록에서 빼면 됩니다.** 목록에서 빠지면 agent에 노출되지 않고, 남은 메인 tool만으로 앱이 동작합니다. (기준 저장소는 모든 tool을 구현·등록한 상태를 유지합니다.)
- **주차 간 참조 규칙**: 이후 주차가 이전 주차의 tool을 재사용하는 것은 허용되지만, 참조하는 tool은 반드시 이전 주차의 **메인 과제**여야 합니다. 이전 주차의 추가 과제를 이후 주차 메인 흐름이 참조하면 안 됩니다.
- **공지 필요 (티어 재분류)**: 다음 tool은 이후 주차 메인 흐름이 참조하므로 **메인 과제로 간주**합니다(반드시 구현·유지). 파일 상단 가이드 주석 문구와 무관하게 아래 기준을 우선합니다.
  - Week 2 구조화 bridge `extract_structured_request` / `extract_schedule_request` → Week 3 이후 저장 흐름의 선행 조건 (메인)
  - Week 5 `list_shared_schedules` / `collect_member_schedules` → Week 6 Kana 하위 agent가 그대로 재사용 (메인)
  - 이미 배포된 주차이므로 수강생에게 이 티어 기준을 공지하세요.
...
Week 2 단일 agent가 tool 호출 없이 자연어 요청을 `StructuredRequest` schema에 맞는 structured output으로 반환하게 한다. 구조화 bridge(`extract_structured_request`, `extract_schedule_request`)는 Week 3 이후 저장 tool chain의 선행 조건이므로 **메인 과제**에 포함한다(Week 2에는 별도 추가 과제가 없다).
...
- (선행 조건) Week 2 메인 과제 `extract_schedule_request`가 구현되어 있어야 저장 흐름이 시작됩니다.
- `save_structured_request`, `list_saved_requests`, `get_saved_request`, `personal_list_saved_schedules`로 구조화된 요청을 SQLite에 저장하고 저장된 요청·일정을 조회한다.

### 추가 과제

- `personal_update_saved_schedule`, `personal_delete_saved_schedules`로 저장된 일정을 수정·삭제한다.
- `personal_create_schedule`(Week 1 호환 이중 기록)과 레거시 payload 정규화(`unwrap_legacy_payload`)를 구현한다.
- 구현하지 않으려면 `week03_tools()` 목록에서 위 tool 줄을 빼면 되고, 남은 메인 tool만으로 앱이 동작한다.
...
메인 경로에서 `save_structured_request`는 LLM이 넘긴 flat 필드(`kind`/`title`/`date`/…)를 그대로 저장합니다. 추가 과제인 레거시 payload 정규화(`SaveStructuredRequestInput.unwrap_legacy_payload`)와 helper(`_save_input_from`)는 예전 trace의 `{"payload": …}`/`structured_request` wrapper를 풀어 줍니다. 멘토는 SQLite에 들어간 `raw_json`이 `extract_schedule_request`의 outer payload가 아니라 내부 `structured_request` 기준으로 저장되는지 확인합니다(추가 과제 검증 시).
...
`add_personal_reference`, `search_personal_references`, `search_saved_requests`로 개인 참고자료와 저장된 요청을 출처별로 검색해 RAG 근거 payload를 만든다.

### 추가 과제

`search_conversation_messages`로 앱 채팅 발화를 ChromaDB에 lazy sync해 검색하는 agentic RAG를 구현한다. 구현하지 않으려면 `week04_tools()` 목록에서 이 tool 줄을 빼면 되고, 남은 메인 검색 tool만으로 앱이 동작한다.
...
`search_previous_conversations`, `load_conversation_messages`, `extract_schedules_from_history`로 외부 대화 기록을 MCP wrapper tool로 다루고, `list_shared_schedules`(공유 일정 조회)와 `collect_member_schedules`(내 일정+외부 멤버 busy-time 수집)를 구현한다. 뒤 두 tool은 Week 6 Kana 하위 agent가 그대로 재사용하는 연결 지점이라 메인 과제다.

### 추가 과제

`create_shared_schedule`, `delete_shared_schedule`로 공유 일정 저장소에 row를 직접 등록·삭제한다. 구현하지 않으려면 `week05_tools()` 목록에서 이 두 tool 줄을 빼면 되고, 남은 메인 tool만으로 앱이 동작한다.
...
| 일정 통합 | 내 일정과 외부 멤버 일정이 함께 정리되는지 확인한다. 앱 DB에 저장된 개인 일정뿐 아니라 그룹 일정도 "나" row로 들어오는지 본다. |
...
- 저장된 내 일정을 `kind="personal_schedule"`로만 읽어, 이번 조율 대상에 없는 멤버와 잡아둔 그룹 일정이 내 바쁜 시간에서 빠지는 경우 (Week 6에서 이미 찬 시간이 후보로 제시된다)
...
- 저장된 내 일정은 개인/그룹 구분 없이 모두 내 바쁜 시간이라는 점을 짚어준다. 그룹 일정은 저장 시 참석자별 공유 row로도 동기화되지만, 그 참석자가 이번 조율 대상에 없으면 외부 조회로는 잡히지 않는다.
...
- 멤버별 일정 정보가 정리된다.
- 내 일정(개인/그룹)과 외부 멤버 일정이 함께 rows 형태로 수집된다.
...
`nana_agent`, `kana_agent`를 구현해 supervisor가 요청에 맞는 하위 agent로 위임하는 뼈대를 완성한다. Kana의 메인 도구(외부 대화 검색/로드, 일정 추출, 공유 일정 조회, 멤버 일정 수집)는 전부 Week 2~5 메인 과제 tool 재사용이므로, 메인 과제만 구현해도 supervisor 위임과 Kana의 멤버 일정 정리까지 동작한다.

### 추가 과제

`find_common_available_slots`(공통 가능 시간 후보 검증), `decide_final_slot`(최종 시간 확정)을 구현해 그룹 일정 조율을 마무리한다. **선행 조건**: Week 5 메인 과제 `collect_member_schedules`/`list_shared_schedules`가 필요하다. 구현하지 않으려면 `kana_tools()` 목록에서 이 두 tool 줄을 빼면 되고, 그러면 Kana는 멤버별 바쁜 시간 근거 정리까지만 처리한다(supervisor 위임 뼈대는 그대로 동작).
...
| Tool 노출 | Nana는 `week04_tools()`, Kana는 `kana_tools()`, supervisor는 `supervisor_tools()`의 위임 도구만 볼 수 있는지 확인한다. (추가 과제를 뺀 수강생은 `kana_tools()`에서 `find_common_available_slots`/`decide_final_slot`이 빠져 있을 수 있다.) |
...
| 후보 검증 (추가) | `find_common_available_slots`가 Kana agent가 고른 `candidate_slots`를 검증하고 근거 rows를 보존하는지 확인한다. |
| 최종 제안 (추가) | `decide_final_slot`이 `final_slot`, 이유, 후보 목록을 payload로 반환하는지 확인한다. |
...
- (메인) Nana/Kana 역할이 명확히 분리되어 있다.
- (메인) supervisor가 요청 유형에 따라 적절한 agent를 선택한다.
- (메인) 그룹 요청에서 Kana가 멤버 일정 수집까지 이어간다.
- (추가) `find_common_available_slots`와 `decide_final_slot`이 busy rows와 candidate payload를 근거로 이어지고, 최종 payload에 `final_slot`, `reason`, `candidates`가 포함된다.
- trace에서 agent와 Tool 호출 흐름을 확인할 수 있다.
- `./run.sh --golden` 또는 최종 통합 검증을 수행했다(메인만 구현 시 추가 과제 케이스는 skip 처리됨).
